<template>
  <div class="p-6 max-w-7xl mx-auto bg-tertiary">
    <div class="bg-primary text-white rounded-lg p-4 w-64 shadow-md relative overflow-hidden">
    ...
    <div class="flex justify-between items-center mb-2 mt-10">
      ...
      <div class="flex gap-3">
        <!-- <button @click="printTable"
          class="bg-primary text-white px-4 py-2 rounded-lg text-sm font-medium shadow non-print">
      ...
      <table class="min-w-full text-sm text-quaternary">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-4 text-left font-semibold">Tabel Ke</th>
        ...
        <tbody>
          ...
          <tr v-for="(order, index) in paginatedOrders" :key="order._id" class="hover:bg-gray-50 transition">
            ...
            <td class="px-6 py-4 text-green-700 font-semibold">{{ order.payment_type }}</td>
            <td class="px-6 py-4">
            ...
            <td class="px-6 py-4 space-y-3 ">
              ...
              <div class="flex items-center gap-3">
                <!-- View Detail Button -->
                <router-link :to="`/payment/${order.order_id}`" class="group bg-green-50 hover:bg-green-100 text-green-600 hover:text-green-700 px-4 py-2 rounded-lg
                  ...
                  <v-icon name="bi-printer" scale="1" />
                </router-link>
    ...
    <div v-if="filteredOrders?.length" class="flex justify-between items-center mt-6 text-sm">
      ...
      <div class="flex gap-2">
        <button @click="prevPage" :disabled="currentPage === 1" class="px-3 py-1 rounded border bg-primary">
          <v-icon name="md-skipprevious-round" scale="1.5" />
        </button>
        <span class="px-3 py-1 text-quaternary">Halaman {{ currentPage }} dari {{ totalPages }}</span>
        <button @click="nextPage" :disabled="currentPage === totalPages" class="px-3 py-1 rounded border bg-primary">
